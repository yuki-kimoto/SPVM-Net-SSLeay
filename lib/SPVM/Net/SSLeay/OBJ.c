...
int32_t SPVM__Net__SSLeay__OBJ__txt2nid(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!success) {
    ...
    env->die(env, stack, "[OpenSSL Error]OBJ_txt2nid failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__OBJ__nid2obj(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!oid) {
    ...
    env->die(env, stack, "[OpenSSL Error]OBJ_nid2obj failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
  ...
  SPVM_OBJ* obj_address_oid = env->new_pointer_object_by_name(env, stack, "Address", oid, &error_id, __func__, FILE_NAME, __LINE__);
  if (error_id) { return error_id; }
...
int32_t SPVM__Net__SSLeay__OBJ__obj2nid(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!success) {
    ...
    env->die(env, stack, "[OpenSSL Error]OBJ_obj2nid failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
