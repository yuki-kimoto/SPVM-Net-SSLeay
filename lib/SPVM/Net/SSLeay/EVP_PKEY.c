...
int32_t SPVM__Net__SSLeay__EVP_PKEY__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]EVP_PKEY_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
  ...
  SPVM_OBJ* obj_self = env->new_pointer_object_by_name(env, stack, "Net::SSLeay::EVP_PKEY", self, &error_id, __func__, FILE_NAME, __LINE__);
  if (error_id) { return error_id; }
