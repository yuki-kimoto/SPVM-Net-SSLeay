...
int32_t SPVM__Net__SSLeay__ASN1_GENERALIZEDTIME__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]ASN1_GENERALIZEDTIME_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
  ...
  SPVM_OBJ* obj_self = env->new_pointer_object_by_name(env, stack, "Net::SSLeay::ASN1_GENERALIZEDTIME", self, &error_id, __func__, FILE_NAME, __LINE__);
  if (error_id) { return error_id; }
...
int32_t SPVM__Net__SSLeay__ASN1_GENERALIZEDTIME__set(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!ret) {
    ...
    env->die(env, stack, "[OpenSSL Error]ASN1_GENERALIZEDTIME_set failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__ASN1_GENERALIZEDTIME__print(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(status == 1)) {
    ...
    env->die(env, stack, "[OpenSSL Error]ASN1_GENERALIZEDTIME_print failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
