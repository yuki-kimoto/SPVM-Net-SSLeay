...
int32_t SPVM__Net__SSLeay__X509_STORE__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]X509_STORE_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__X509_STORE__set_flags(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(status == 1)) {
    ...
    env->die(env, stack, "[OpenSSL Error]X509_STORE_set_flags failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__X509_STORE__add_crl(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(status == 1)) {
    ...
    env->die(env, stack, "[OpenSSL Error]X509_STORE_add_crl failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
