...
int32_t SPVM__Net__SSLeay__BIO__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]BIO_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__BIO__read(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (read_length < 0) {
    ...
    env->die(env, stack, "[OpenSSL Error]BIO_read failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__BIO__write(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (write_length < 0) {
    ...
    env->die(env, stack, "[OpenSSL Error]BIO_write failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__BIO__new_file(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]BIO_new_file failed:%s. $filename:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string, filename);
