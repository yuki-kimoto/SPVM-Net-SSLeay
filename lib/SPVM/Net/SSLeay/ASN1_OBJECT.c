...
int32_t SPVM__Net__SSLeay__ASN1_OBJECT__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die(env, stack, "[OpenSSL Error]ASN1_OBJECT_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
  ...
  SPVM_OBJ* obj_self = env->new_pointer_object_by_name(env, stack, "Net::SSLeay::ASN1_OBJECT", self, &error_id, __func__, FILE_NAME, __LINE__);
  if (error_id) { return error_id; }
