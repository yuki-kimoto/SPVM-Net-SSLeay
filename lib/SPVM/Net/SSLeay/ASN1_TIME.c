...
int32_t SPVM__Net__SSLeay__ASN1_TIME__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!self) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]ASN1_TIME_new failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__ASN1_TIME__set(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!ret) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]ASN1_TIME_set failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__ASN1_TIME__print(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!obj_b) {
    return env->die_v2(env, stack, "The BIO object $b must be defined.", __func__, FILE_NAME, __LINE__);
  }
  ...
  if (!(status == 1)) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]ASN1_TIME_print failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__ASN1_TIME__to_tm(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!obj_tm) {
    return env->die_v2(env, stack, "The Sys::Time::Tm object $tm must be defined.", __func__, FILE_NAME, __LINE__);
  }
  ...
  if (!(status == 1)) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]ASN1_TIME_to_tm failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
...
int32_t SPVM__Net__SSLeay__ASN1_TIME__to_generalizedtime(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!gtime) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]ASN1_TIME_to_generalizedtime failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
