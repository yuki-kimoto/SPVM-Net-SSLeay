...
int32_t SPVM__Net__SSLeay__RAND__load_file(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!obj_filename) {
    return env->die_v2(env, stack, "The filename $filename must be defined.", __func__, FILE_NAME, __LINE__);
  }
  ...
  if (!(read_length >= 0)) {
    ...
    env->die_v2(env, stack, "[OpenSSL Error]RAND_load_file failed:%s.", __func__, FILE_NAME, __LINE__, ssl_error_string);
